...
class EventBus {
 ...
 protected:
  template <typename EVENT>
  void add(Listener<EVENT>* l);

  template <typename EVENT>
  void remove(Listener<EVENT>* l);
...
template <typename EVENT>
void EventBus::add(EventBus::Listener<EVENT>* l) {
  m_dispatchers.lock();
...
template <typename EVENT>
void EventBus::remove(EventBus::Listener<EVENT>* l) {
  m_dispatchers.lock();
  if (EventBus::EventDispatcher<EVENT>* dispatcher =
          dynamic_cast<EventDispatcher<EVENT>*>(
              m_dispatchers->find(std::type_index(typeid(EVENT)))->second))
